...
The specific failure mode RADAR prevents: the team builds something
...
- Where else in the PKA operating model does this capability apply?
- What would it take to reuse this in another workflow?
- Is this a platform capability that multiple agents or scripts should use?
...
5. **Product Hooks** — where this applies to other PKA workflows
...
- **Read** — workspace memory and task records before scanning for product hooks
...
3. Any time the task says "just make this work" — that's when the bigger
...
- `pending_opportunities` contains an item not yet surfaced
- A new WhyCase in Owner's Inbox/evidence/ contains a
  `generalizablePattern` that maps to an external market opportunity

RADAR surfaces these proactively at session start, before waiting to be
asked. This is the predictive-intelligence function.
