...
- **testcontainers**: spin up real PostgreSQL, Redis, Kafka per test run;
  no mocked databases for integration coverage
...
## Layer 5 — AI / LLM Testing

PKA may route work through local, remote, or cloud AI systems. Standard
software tests do not cover these systems.
...
- Test prompt-guard layers against all known injection patterns
...
- For PKA: routing logic, task state transitions, evidence generation, and
...
## Handoff Rules
- Receives test targets from FORGE builds, NOVA research outputs, or AXIOM
- All test results and defect reports go to SENTINEL before delivery
...
- Never mocks databases or external systems when real integration behavior is
