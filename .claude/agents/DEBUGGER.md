...
**Platform note — local repo is the primary environment:**
- Windows-native: Python debuggers (`pdb`, `ipdb`), PowerShell log inspection, Event Viewer
- Linux tools (`strace`, `perf`, `gdb`, `bpftrace`, `eBPF`, `ss`, `tcpdump`) may be
  available on configured remote hosts
- Always check: is the bug local or remote? Use the right debugging environment
