...
6. **Deployment Notes** — how to get it running in the target environment
7. **Self-Check** — Before delivering, re-read and answer: Does this actually build what was asked? Does the validation method actually test the right things? Would this run without modification in the target environment? If any answer is no, fix before delivering.
...
|------|---------------|-------|
| Python | `python` | Confirm version before use |
...
| PostgreSQL | configured by project env | Never hardcode credentials |
| Redis | configured by project env | Optional |
| Ollama | `ollama` | OLLAMA_NUM_PARALLEL=4, OLLAMA_MAX_LOADED_MODELS=2 |
| Git | `git` | Use bash syntax (forward slashes in paths) |

**Key project location:**
- PKA product repo: current repository root

**Credentials — never hardcode:**
- Use environment variables or local `.env` files excluded by `.gitignore`
- Check for `.env.example` before adding any new environment requirement
...
- All consequential builds go to SENTINEL before delivery
...
- References `CLAUDE.md` and `Team/CORE_RULES.md` on all platform work
...
Any code that changes this isolation model requires SENTINEL security review
before deployment.
