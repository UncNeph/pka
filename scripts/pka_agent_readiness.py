...
def main() -> int:
    ...
    connectivity = [
        ...
        ("runtime_messaging", exists(ROOT / "scripts" / "pka_message_cli.py"), "Runtime messaging CLI exists"),
        ("ssh_key_present", any_exists([Path.home() / ".ssh" / "id_rsa", Path.home() / ".ssh" / "id_ed25519"]), "Optional SSH key available"),
    ...
    strengths = [
        "Connectivity is strong across inboxes, hooks, runtime messaging, and structured agent messaging",
        "Context is strong across repo memory, ledger, journals, handoff, status, and dream consolidation",
