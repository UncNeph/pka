...
Autonomous LLM runtime for the PKA workspace. Powered by local models,
environment-configured remote model endpoints, or cloud OpenAI-compatible LLMs.
...
- **Provider routing**: local Ollama, environment-configured remote endpoints, and cloud OpenAI-compatible APIs
...
| gemma3 | Remote/local endpoint | `qwen3:latest` | Fast summarization/classification alias |
| qwen3-8b | Remote/local endpoint | `qwen3:latest` | Fast tool-calling |
| qwen3 | Remote/local endpoint | `qwen3-fast-hermes:latest` | Tool-calling default |
| llama70b | Remote/local endpoint | `llama3.1:70b` | Deep reasoning/analysis |
| deepseek | Remote/local endpoint | `deepseek-r1:32b` | Step-by-step reasoning |
...
| `ssh_exec` | SSH to optional configured remote hosts |

## Safety

- Writes blocked outside `C:\Users\techai\PKA testing\`
- CLAUDE.md and root MEMORY.md → read-only
...
Remote primary: set `AGENT_BRAIN_SPARK1`
Remote fallback: set `AGENT_BRAIN_TUNNEL`
...
Data isolation rule: cloud models are available only when explicitly selected
or configured. Do not send `CLAUDE.md`, root `MEMORY.md`, Team memory, task
records, or Owner's Inbox contents to public/cloud models. Agent Brain
withholds its local memory block from cloud providers by default; set
`AGENT_BRAIN_ALLOW_CLOUD_MEMORY=1` only when that is intentional.
