"""SSH remote execution tools for optional external hosts."""
...
@tool(
    name="ssh_exec",
    description=(
        "Execute a shell command on an optional external host configured by environment variables. "
        "Use this to check services, run model commands, or view logs. "
    ...
    parameters={
        ...
        "properties": {
            "host": {
                "type": "string",
                "enum": ["spark1", "spark2"],
                "description": "Target host alias configured through Agent Brain SSH environment variables.",
            },
            "command": {
                "type": "string",
...
async def ssh_exec(host: str, command: str, timeout: int = SSH_TIMEOUT_S) -> str:
    ...
    if not SPARK1_SSH_HOST or not SPARK1_SSH_USER:
        return "[ERROR] SSH host is not configured. Set AGENT_BRAIN_SPARK1_SSH_HOST and AGENT_BRAIN_SPARK1_SSH_USER."

    if host == "spark1":
        ssh_args = [f"{SPARK1_SSH_USER}@{SPARK1_SSH_HOST}"]
    elif host == "spark2":
        if not SPARK2_SSH_HOST or not SPARK2_SSH_USER:
            return "[ERROR] SSH host is not configured. Set AGENT_BRAIN_SPARK2_SSH_HOST and AGENT_BRAIN_SPARK2_SSH_USER."
        ssh_args = [
